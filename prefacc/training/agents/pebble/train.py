...
def _init_training_state(
    ...
    reward_model_network, # TODO add type
    alpha_optimizer: optax.GradientTransformation,
    policy_optimizer: optax.GradientTransformation,
    q_optimizer: optax.GradientTransformation,
) -> TrainingState:
  """Inits the training state and replicates it over devices."""
  key_policy, key_q, key_reward = jax.random.split(key, 3)
  log_alpha = jnp.asarray(0.0, dtype=jnp.float32)
  alpha_optimizer_state = alpha_optimizer.init(log_alpha)

  reward_model_params = reward_model_network.reward_model_network.init(key_reward)
  ...
  training_state = TrainingState(
      ...
      reward_model_optimizer_state=reward_model_optimizer.init(reward_model_params),
...
def train(
  ...
  reward_model_optimizer = optax.adam(rm_learning_rate) # for ant
  ...
  # logging.info("Training state: %s", training_state)
